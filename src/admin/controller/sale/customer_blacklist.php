...
class ControllerSaleCustomerBlacklist extends Controller
{
	...
	public function insert()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm())
		{
			...
			$this->redirect($this->url->link('sale/customer_blacklist', $url, 'SSL'));
		}
	...
	private function getList()
	{
		...
		$this->data['breadcrumbs'][]       = array(
			...
			'href'      => $this->url->link('sale/customer_blacklist', $url, 'SSL'),
			'separator' => ' :: '
		);
		$this->data['insert']              = $this->url->link('sale/customer_blacklist/insert', $url, 'SSL');
		$this->data['delete']              = $this->url->link('sale/customer_blacklist/delete', $url, 'SSL');
		...
		foreach ($results as $result)
		{
			...
			$action[]                            = array(
				...
				'href' => $this->url->link('sale/customer_blacklist/update', '&customer_ip_blacklist_id=' . $result['customer_ip_blacklist_id'] . $url, 'SSL')
			);
			$this->data['customer_blacklists'][] = array(
		...
		$this->data['sort_ip'] = $this->url->link('sale/customer_blacklist', '&sort=ip' . $url, 'SSL');
		$url                   = '';
		...
		$pagination->url          = $this->url->link('sale/customer_blacklist', $url . '&page={page}', 'SSL');
		$this->data['pagination'] = $pagination->render();
		$this->data['sort']       = $sort;
		$this->data['order']      = $order;
		$this->template           = 'template/sale/customer_blacklist_list.tpl';
		$this->children           = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm()
	{
		...
		{
			$this->data['action'] = $this->url->link('sale/customer_blacklist/insert', $url, 'SSL');
		}
		else
		{
			$this->data['action'] = $this->url->link('sale/customer_blacklist/update', '&customer_ip_blacklist_id=' . $this->request->get['customer_ip_blacklist_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('sale/customer_blacklist', $url, 'SSL');
		...
		$this->template = 'template/sale/customer_blacklist_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
