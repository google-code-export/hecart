...
class ControllerSaleCustomer extends Controller
{
	...
	public function insert()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm())
		{
			...
			$this->redirect($this->url->link('sale/customer', $url, 'SSL'));
		}
	...
	private function getList()
	{
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('sale/customer', $url, 'SSL'),
			'separator' => ' :: '
		);
		$this->data['approve']       = $this->url->link('sale/customer/approve', $url, 'SSL');
		$this->data['insert']        = $this->url->link('sale/customer/insert', $url, 'SSL');
		$this->data['delete']        = $this->url->link('sale/customer/delete', $url, 'SSL');
		...
		$this->data['sort_name']           = $this->url->link('sale/customer', '&sort=name' . $url, 'SSL');
		$this->data['sort_email']          = $this->url->link('sale/customer', '&sort=c.email' . $url, 'SSL');
		$this->data['sort_customer_group'] = $this->url->link('sale/customer', '&sort=customer_group' . $url, 'SSL');
		$this->data['sort_status']         = $this->url->link('sale/customer', '&sort=c.status' . $url, 'SSL');
		$this->data['sort_approved']       = $this->url->link('sale/customer', '&sort=c.approved' . $url, 'SSL');
		$this->data['sort_ip']             = $this->url->link('sale/customer', '&sort=c.ip' . $url, 'SSL');
		$this->data['sort_date_added']     = $this->url->link('sale/customer', '&sort=c.date_added' . $url, 'SSL');
		$url                               = '';
		...
		$pagination->url                        = $this->url->link('sale/customer', $url . '&page={page}', 'SSL');
		...
		$this->template       = 'template/sale/customer_list.tpl';
		$this->children       = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm()
	{
		...
		if (!isset($this->request->get['customer_id']))
		{
			$this->data['action'] = $this->url->link('sale/customer/insert', $url, 'SSL');
		}
		else
		{
		...
		$this->data['cancel'] = $this->url->link('sale/customer', $url, 'SSL');
		...
		$this->template = 'template/sale/customer_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function login()
	{
		...
		if ($customer_info)
		{
			...
			{
				$this->redirect(HTTP_STORE . 'index.php?route=account/login&token=' . $token);
			}
		}
		else
		{
	...
	public function transaction()
	{
		...
		$this->data['pagination'] = $pagination->render();
		$this->template           = 'template/sale/customer_transaction.tpl';
		$this->response->setOutput($this->render());
	}
	...
	public function reward()
	{
		...
		$this->data['pagination'] = $pagination->render();
		$this->template           = 'template/sale/customer_reward.tpl';
		$this->response->setOutput($this->render());
	}
