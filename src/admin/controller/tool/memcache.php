...
class ControllerToolMemcache extends Controller
{
	...
	public function index()
	{
		...
		$this->document->setTitle($this->language->get('Oper Memcache'));
		$this->data['heading_title'] = 'Oper Memcache';
		...
		$this->template = 'template/tool/memcache.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function execDoCommend($op)
	{
		...
		switch ($op)
		{
			case 1: // host stats
				...
				$this->data['content'] .= '<div class="info"><h2>General Cache Information</h2>';
				$this->data['content'] .= '<table class="list"><tbody><tr><td width="28%">PHP Version</td><td>' . $phpversion . '</td></tr>';
				$this->data['content'] .= "<tr><td>Memcached Host" . ((count($mem_servers) > 1) ? 's' : '') . "</td><td>";
				$i = 0;
				if (!isset($_GET['singleout']) && count($mem_servers) > 1)
				{
					foreach ($mem_servers as $server)
					{
						$this->data['content'] .= ($i + 1) . '. <a href="' . $self_url . '&singleout=' . $i++ . '">' . $server . '</a><br/>';
					}
				}
				else
				{
					$this->data['content'] .= '1.' . $mem_servers[0];
				}

				if (isset($_GET['singleout']))
				{
					$this->data['content'] .= '<a href="' . $self_url . '">(all servers)</a><br/>';
				}

				$this->data['content'] .= "</td></tr>\n";
				$this->data['content'] .= "<tr><td>Total Memcache Cache</td><td>" . $this->bsize($memcacheStats['limit_maxbytes']) . "</td></tr>\n";
				$this->data['content'] .= '</tbody></table></div><div class="info div1"><h2>Memcache Server Information</h2>';

				foreach ($mem_servers as $server)
				{
					$this->data['content'] .= '<table class="list"><tbody>';
					$this->data['content'] .= '<tr><td width="28%">' . $server . '</td><td><a href="' . $self_url . '&server=' . array_search($server, $mem_servers) . '&op=6">[<b>Flush this server</b>]</a></td></tr>';
					$this->data['content'] .= '<tr><td>Start Time</td><td>' . date(DATE_FORMAT, $memcacheStatsSingle[$server]['STAT']['time'] - $memcacheStatsSingle[$server]['STAT']['uptime']) . '</td></tr>';
					$this->data['content'] .= '<tr><td>Uptime</td><td>' . $this->duration($memcacheStatsSingle[$server]['STAT']['time'] - $memcacheStatsSingle[$server]['STAT']['uptime']) . '</td></tr>';
					$this->data['content'] .= '<tr><td>Memcached Server Version</td><td>' . $memcacheStatsSingle[$server]['STAT']['version'] . '</td></tr>';
					$this->data['content'] .= '<tr><td>Used Cache Size</td><td>' . $this->bsize($memcacheStatsSingle[$server]['STAT']['bytes']) . '</td></tr>';
					$this->data['content'] .= '<tr><td>Total Cache Size</td><td>' . $this->bsize($memcacheStatsSingle[$server]['STAT']['limit_maxbytes']) . '</td></tr>';
					$this->data['content'] .= '</tbody></table>';
				}

				$this->data['content'] .= '</div><div class="graph div3"><h2>Host Status Diagrams</h2><table class="list"><tbody>';
				$size = 'width=' . (GRAPH_SIZE + 50) . ' height=' . (GRAPH_SIZE + 10);
				$this->data['content'] .= '<tr><td>Cache Usage</td><td>Hits &amp; Misses</td></tr>';

				if ($this->graphics_avail())
				{
					$this->data['content'] .= "<tr><td><img {$size} src=\"{$self_url}&IMG=1&" . (isset($_GET['singleout']) ? 'singleout = ' . $_GET['singleout'] . ' & ' : '') . "{$time}\"></td><td><img {$size} src=\"{$self_url}&IMG=2&" . (isset($_GET['singleout']) ? 'singleout = ' . $_GET['singleout'] . ' & ' : '') . "{$time}\"></td></tr>\n";
				}

				$this->data['content'] .= '<tr><td><span class="green box" >&nbsp; </span>　Free: ' . $this->bsize($mem_avail) . sprintf(" (%.1f%%)", $mem_avail * 100 / $mem_size) . "</td>\n" . '<td><span class="green box" >&nbsp; </span>　Hits: ' . $hits . sprintf(" (%.1f%%)", $hits * 100 / ($hits + $misses)) . "</td>\n";
				$this->data['content'] .= '</tr><tr><td><span class="red box" >&nbsp; </span>　Used: ' . $this->bsize($mem_used) . sprintf(" (%.1f%%)", $mem_used * 100 / $mem_size) . "</td>\n" . '<td><span class="red box" >&nbsp; </span>　Misses: ' . $misses . sprintf(" (%.1f%%)", $misses * 100 / ($hits + $misses)) . "</td>\n";
				$this->data['content'] .= "</tr></tbody></table>";

				$this->data['content'] .= "<div class=\"info\"><h2>Cache Information</h2>";
				$this->data['content'] .= "<table class=\"list\"><tbody>";
				$this->data['content'] .= "<tr><td>Current Items(total)</td><td>$curr_items ({$total_items})</td></tr>";
				$this->data['content'] .= "<tr><td>Hits</td><td>{$hits}</td></tr>";
				$this->data['content'] .= "<tr><td>Misses</td><td>{$misses}</td></tr>";
				$this->data['content'] .= "<tr><td>Request Rate (hits, misses)</td><td>{$req_rate} cache requests/second</td></tr>";
				$this->data['content'] .= "<tr><td>Hit Rate</td><td>{$hit_rate} cache requests/second</td></tr>";
				$this->data['content'] .= "<tr><td>Miss Rate</td><td>{$miss_rate} cache requests/second</td></tr>";
				$this->data['content'] .= "<tr><td>Set Rate</td><td>{$set_rate} cache requests/second</td></tr>";
				$this->data['content'] .= "</tbody></table>";
			...
			case 2: // variables
				...
				{
					$this->data['content'] .= '<div class="info"><table class="list"><thead><tr><th colspan="2"><b>' . $server . '</b></th></tr><tr><td>Slab Id</td><td>Info</td></tr></thead><tbody>';
					foreach ($entries as $slabId => $slab)
					{
						$dumpUrl = $self_url . '&op=2&server=' . (array_search($server, $mem_servers)) . '&dumpslab=' . $slabId;
						$this->data['content'] .= "<tr><td><center>" . '<a href="' . $dumpUrl . '">' . $slabId . '</a>' . "</center></td><td class=td-last><b>Item count:</b> ";
						$this->data['content'] .= $slab['number'] . '<br/><b>Age:</b>' . $this->duration($time - $slab['age']) . '<br/> <b>Evicted:</b>' . ((isset($slab['evicted']) && $slab['evicted'] == 1) ? 'Yes' : 'No');
						if ((isset($_GET['dumpslab']) && $_GET['dumpslab'] == $slabId) && (isset($_GET['server']) && $_GET['server'] == array_search($server, $mem_servers)))
						{
							$this->data['content'] .= "<br/><b>Items: item</b><br/>";
							...
							foreach ($items['ITEM'] as $itemKey => $itemInfo)
							{
								...
								$this->data['content'] .= '<a href="' . $self_url . '&op=4&server=' . (array_search($server, $mem_servers)) . '&key=' . base64_encode($itemKey) . '">' . $itemKey . '</a>';
								if ($i++ % 10 == 0)
								{
					...
					$this->data['content'] .= '</tbody></table></div>';
				}
				break;
			case 4: //item dump
				if (!isset($_GET['key']) || !isset($_GET['server']))
				{
					$this->data['content'] .= "No key set!";
					break;
				}
				...
				$this->data['content'] .= '<div class="info"><table class="list"><thead><tr><td>Server<td>Key</td><td>Value</td><td>Delete</td></tr></thead>';
				if (!isset($r['VALUE']))
				{
					$this->data['content'] .= "<tbody><tr><td>{$theServer}</td><td>{$theKey}<br/>flag:null <br/>Size: 0</td><td>NULL</td>";
				}
				else
				{
					$this->data['content'] .= "<tbody><tr><td>{$theServer}</td><td>{$theKey}<br/>flag:{$r['VALUE'][$theKey]['stat']['flag']} <br/>Size:";
					$this->data['content'] .= $this->bsize($r['VALUE'][$theKey]['stat']['size']) . "</td><td>" . chunk_split($r['VALUE'][$theKey]['value'], 40) . "</td>";
				}
				$this->data['content'] .= "<td><a href=\"{$self_url}&op=5&server=" . (int)$_GET['server'] . '&key=' . base64_encode($theKey) . "\">Delete</a></td></tr>";
				$this->data['content'] .= '</tbody></table></div>';
				break;
			case 5: // item delete
				if (!isset($_GET['key']) || !isset($_GET['server']))
				...
				$this->data['content'] .= 'Deleting Memcache Key <br/><br/>' . $theKey . '<br/><br/>' . $r;
				break;
			case 6: // flush server
				$theServer = $mem_servers[(int)$_GET['server']];
				$r         = $this->flushServer($theServer);
				$this->data['content'] .= 'Flush  ' . $theServer . ":" . $r;
				break;
		}
	}
	...
	private function menu_entry($ob, $title)
	{
		...
		if ($ob == $_GET['op'])
		{
			return "<a class=\"button child_active\" href=\"{$self_url}&op=$ob\">{$title}</a>";
		}

		return "<a class=\"button active\" href=\"{$self_url}&op=$ob\">{$title}</a>";
	}
	...
	private function getMenu($self_url)
	{
		if ($_GET['op'] != 4)
		{
			$this->data['menu'] .= "<a href=\"{$self_url}&op={$_GET['op']}\" class=\"button\">Refresh 刷新</a>";
		}
		else
		{
			$this->data['menu'] .= "<a href=\"{$self_url}&op=2}\" class=\"button\">Back 后退</a>";
		}

		$this->data['menu'] .= $this->menu_entry(1, 'Host Stats 主机状态') . $this->menu_entry(2, 'Variables 变量');
	}
