...
class ControllerLocalisationGeoZone extends Controller
{
	...
	public function insert()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm())
		{
			...
			$this->redirect($this->url->link('localisation/geo_zone', $url, 'SSL'));
		}
	...
	private function getList()
	{
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('localisation/geo_zone', $url, 'SSL'),
			'separator' => ' :: '
		);
		$this->data['insert']        = $this->url->link('localisation/geo_zone/insert', $url, 'SSL');
		$this->data['delete']        = $this->url->link('localisation/geo_zone/delete', $url, 'SSL');
		...
		foreach ($results as $result)
		{
			...
			$action[]                  = array(
				...
				'href' => $this->url->link('localisation/geo_zone/update', '&geo_zone_id=' . $result['geo_zone_id'] . $url, 'SSL')
			);
			$this->data['geo_zones'][] = array(
		...
		$this->data['sort_name']        = $this->url->link('localisation/geo_zone', '&sort=name' . $url, 'SSL');
		$this->data['sort_description'] = $this->url->link('localisation/geo_zone', '&sort=description' . $url, 'SSL');
		$url                            = '';
		...
		$pagination->url          = $this->url->link('localisation/geo_zone', $url . '&page={page}', 'SSL');
		$this->data['pagination'] = $pagination->render();
		$this->data['sort']       = $sort;
		$this->data['order']      = $order;
		$this->template           = 'template/localisation/geo_zone_list.tpl';
		$this->children           = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm()
	{
		...
		{
			$this->data['action'] = $this->url->link('localisation/geo_zone/insert', $url, 'SSL');
		}
		else
		{
			$this->data['action'] = $this->url->link('localisation/geo_zone/update', '&geo_zone_id=' . $this->request->get['geo_zone_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('localisation/geo_zone', $url, 'SSL');
		...
		$this->template = 'template/localisation/geo_zone_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validateForm()
	{
		...
		if ((mb_strlen($this->request->post['description']) < 3) || (mb_strlen($this->request->post['description']) > 255))
		{
