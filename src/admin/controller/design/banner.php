...
class ControllerDesignBanner extends Controller
{
	...
	public function insert()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm())
		{
			...
			$this->redirect($this->url->link('design/banner', $url, 'SSL'));
		}
	...
	private function getList()
	{
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('design/banner', $url, 'SSL'),
			'separator' => ' :: '
		);
		$this->data['insert']        = $this->url->link('design/banner/insert', $url, 'SSL');
		$this->data['delete']        = $this->url->link('design/banner/delete', $url, 'SSL');
		...
		foreach ($results as $result)
		{
			...
			$action[]                = array(
				...
				'href' => $this->url->link('design/banner/update', '&banner_id=' . $result['banner_id'] . $url, 'SSL')
			);
			$this->data['banners'][] = array(
		...
		$this->data['sort_name']   = $this->url->link('design/banner', '&sort=name' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('design/banner', '&sort=status' . $url, 'SSL');
		$url                       = '';
		...
		$pagination->url          = $this->url->link('design/banner', $url . '&page={page}', 'SSL');
		$this->data['pagination'] = $pagination->render();
		$this->data['sort']       = $sort;
		$this->data['order']      = $order;
		$this->template           = 'template/design/banner_list.tpl';
		$this->children           = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm()
	{
		...
		{
			$this->data['action'] = $this->url->link('design/banner/insert', $url, 'SSL');
		}
		else
		{
			$this->data['action'] = $this->url->link('design/banner/update', '&banner_id=' . $this->request->get['banner_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('design/banner', $url, 'SSL');
		...
		$this->template         = 'template/design/banner_form.tpl';
		$this->children         = array(
			'common/header',
			'common/footer'
		);
	...
	private function validateForm()
	{
		...
		if (isset($this->request->post['banner_image']))
		{
			foreach ($this->request->post['banner_image'] as $banner_image_id => $banner_image)
			{
				...
				{
					if ((mb_strlen($banner_image_description['title']) < 2) || (mb_strlen($banner_image_description['title']) > 64))
					{
