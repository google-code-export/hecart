...
class ControllerCatalogOption extends Controller
{
	...
	public function insert()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm())
		{
			...
			$this->redirect($this->url->link('catalog/option', $url, 'SSL'));
		}
	...
	private function getList()
	{
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/option', $url, 'SSL'),
			'separator' => ' :: '
		);
		$this->data['insert']        = $this->url->link('catalog/option/insert', $url, 'SSL');
		$this->data['delete']        = $this->url->link('catalog/option/delete', $url, 'SSL');
		...
		foreach ($results as $result)
		{
			...
			$action[]                = array(
				...
				'href' => $this->url->link('catalog/option/update', '&option_id=' . $result['option_id'] . $url, 'SSL')
			);
			$this->data['options'][] = array(
		...
		$this->data['sort_name']       = $this->url->link('catalog/option', '&sort=od.name' . $url, 'SSL');
		$this->data['sort_sort_order'] = $this->url->link('catalog/option', '&sort=o.sort_order' . $url, 'SSL');
		$url                           = '';
		...
		$pagination->url          = $this->url->link('catalog/option', $url . '&page={page}', 'SSL');
		$this->data['pagination'] = $pagination->render();
		$this->data['sort']       = $sort;
		$this->data['order']      = $order;
		$this->template           = 'template/catalog/option_list.tpl';
		$this->children           = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm()
	{
		...
		{
			$this->data['action'] = $this->url->link('catalog/option/insert', $url, 'SSL');
		}
		else
		{
			$this->data['action'] = $this->url->link('catalog/option/update', '&option_id=' . $this->request->get['option_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('catalog/option', $url, 'SSL');
		...
		$this->template         = 'template/catalog/option_form.tpl';
		$this->children         = array(
			'common/header',
			'common/footer'
		);
	...
	private function validateForm()
	{
		...
		{
			if ((mb_strlen($value['name']) < 1) || (mb_strlen($value['name']) > 128))
			{
		...
		if (isset($this->request->post['option_value']))
		{
			foreach ($this->request->post['option_value'] as $option_value_id => $option_value)
			{
				...
				{
					if ((mb_strlen($option_value_description['name']) < 1) || (mb_strlen($option_value_description['name']) > 128))
					{
