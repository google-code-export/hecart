...
class ControllerCatalogDownload extends Controller
{
	...
	public function insert()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm())
		{
			...
			$this->redirect($this->url->link('catalog/download', $url, 'SSL'));
		}
	...
	private function getList()
	{
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/download', $url, 'SSL'),
			'separator' => ' :: '
		);
		$this->data['insert']        = $this->url->link('catalog/download/insert', $url, 'SSL');
		$this->data['delete']        = $this->url->link('catalog/download/delete', $url, 'SSL');
		...
		foreach ($results as $result)
		{
			...
			$action[]                  = array(
				...
				'href' => $this->url->link('catalog/download/update', '&download_id=' . $result['download_id'] . $url, 'SSL')
			);
			$this->data['downloads'][] = array(
		...
		$this->data['sort_name']      = $this->url->link('catalog/download', '&sort=dd.name' . $url, 'SSL');
		$this->data['sort_remaining'] = $this->url->link('catalog/download', '&sort=d.remaining' . $url, 'SSL');
		$url                          = '';
		...
		$pagination->url          = $this->url->link('catalog/download', $url . '&page={page}', 'SSL');
		$this->data['pagination'] = $pagination->render();
		$this->data['sort']       = $sort;
		$this->data['order']      = $order;
		$this->template           = 'template/catalog/download_list.tpl';
		$this->children           = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm()
	{
		...
		{
			$this->data['action'] = $this->url->link('catalog/download/insert', $url, 'SSL');
		}
		else
		{
			$this->data['action'] = $this->url->link('catalog/download/update', '&download_id=' . $this->request->get['download_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('catalog/download', $url, 'SSL');
		...
		$this->template = 'template/catalog/download_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validateForm()
	{
		...
		{
			if ((mb_strlen($value['name']) < 3) || (mb_strlen($value['name']) > 64))
			{
				$this->error['name'][$language_id] = $this->language->get('error_name');
			}
		}

		if ((mb_strlen($this->request->post['filename']) < 3) || (mb_strlen($this->request->post['filename']) > 128))
		{
			$this->error['filename'] = $this->language->get('error_filename');
		}

		if (!file_exists(DIR_ROOT . '/www/download/' . $this->request->post['filename']) && !is_file(DIR_DOWNLOAD . $this->request->post['filename']))
		{
			$this->error['filename'] = $this->language->get('error_exists');
		}

		if ((mb_strlen($this->request->post['mask']) < 3) || (mb_strlen($this->request->post['mask']) > 128))
		{
	...
	public function upload()
	{
		...
		if (!isset($json['error']))
		{
			if (is_uploaded_file($this->request->files['file']['tmp_name']) && file_exists($this->request->files['file']['tmp_name']))
			{
				...
				move_uploaded_file($this->request->files['file']['tmp_name'], DIR_ROOT . "/www/download/{$filename}.{$ext}");
			}
