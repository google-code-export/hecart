 ...
 * 版本: $Id: db.php 1 2012-11-20 05:55:12Z Administrator $
...
class wcore_db
{
	 ...
	 *
	 * @var string 字符串,可以接受的值有:utf8 gb2312 gbk
	 */
	 ...
	 * @param string $where 查询条件
	 * @param string $order 排序
	 * @return mixed 有数据获取返回数据没有则返回flase
	 */
	public function opt_one($field = '*', $where = '', $order = '')
	{
		$res = $this->opt_row($field, $where, $order);
		if (empty($res))
		{
			return false;
		}
	...
	{
		$res = $this->opt_all($field, $where, $order);
		if (empty($res))
		{
			$false = false;
	 ...
	 * @return mixed　失败成返回false，若表有自动编号则返回自动编号，若无则返回影响多少行
	 */
	public function opt_insert(&$data)
	{
		...
		$sql = $this->make_sql($data);
		$sql = "INSERT INTO {$this->_opt} {$sql};";
		if (defined('SQL_DEBUG') && SQL_DEBUG)
		{
			echo("{$sql}<br/>");
		}
		...
		$this->dbexec($sql);
		if ($aff_rows = $this->affected_rows())
		{
			$insert_id = $this->insert_id();

			return $insert_id ? $insert_id : $aff_rows;
		}
	...
	public function opt_update(&$data, $where = '')
	{
		...
		$sql = "UPDATE {$this->_opt} SET {$sql}{$where};";
		if (defined('SQL_DEBUG') && SQL_DEBUG)
		{
			echo("{$sql}<br/>");
		}
	...
	public function opt_del($where)
	{
		if (!empty($where)) //删除记录
		{
			$sql = "DELETE QUICK FROM {$this->_opt} WHERE {$where};";
			if (defined('SQL_DEBUG') && SQL_DEBUG)
			{
				echo("{$sql}<br/>");
			}
	 ...
	 */
	public function &make_sql(&$arr, $type = 'i')
	{
		$key   = '';
	 ...
	 * @return mixed 执行成功有数据则返回数据,失败返回flase
	 */
	 ...
	 * @return bool|int 插入成功成返回自动编号，失败返回false
	 */
	public function insert($opt, &$data, $show_err = true)
	{
		...
		$sql = $this->make_sql($data);
		$sql = "INSERT INTO {$opt} {$sql}";
		$this->dbexec($sql, $show_err);

		return $this->affected_rows() ? $this->insert_id() : false;
	}
	...
	{
		if ((defined('SQL_ERROR_FOR_TIP') && SQL_ERROR_FOR_TIP) || strpos($db_err, 'Duplicate entry') !== false)
		{
			trigger_error("{$db_err}<br/><br/>{$msg}", E_USER_ERROR);
		}
