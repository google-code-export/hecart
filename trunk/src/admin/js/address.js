...
{
	$.ajax({
		url: 'index.php?route=localisation/country/country&country_id=' + $country_id,
		dataType: 'json',
		beforeSend: function()
		{
		...
		success: function(json)
		{
			...
			{
				$('#' + slt_city_obj).html('<select name="' + $slt_city_id + '" class="large-field"></select>');
			...
			else
			{
				...
				$('#' + slt_city_obj).html('<input type="text" name="' + $slt_city_id + '" value="' + $city_val + '" class="large-field" />');
			}
...
function loadCity($zone_id, $slt_zone_id, $slt_city_id)
{
	...
	$.ajax({
		url: 'index.php?route=localisation/zone/zone&zone_id=' + $zone_id,
		dataType: 'json',
		beforeSend: function()
		{
