...
class ControllerCatalogInformation extends Controller
{
	...
	public function insert()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm())
		{
			...
			$this->redirect($this->url->link('catalog/information', $url, 'SSL'));
		}
	...
	private function getList()
	{
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/information', $url, 'SSL'),
			'separator' => ' :: '
		);
		$this->data['insert']        = $this->url->link('catalog/information/insert', $url, 'SSL');
		$this->data['delete']        = $this->url->link('catalog/information/delete', $url, 'SSL');
		...
		foreach ($results as $result)
		{
			...
			$action[]                     = array(
				...
				'href' => $this->url->link('catalog/information/update', '&information_id=' . $result['information_id'] . $url, 'SSL')
			);
			$this->data['informations'][] = array(
		...
		$this->data['sort_title']      = $this->url->link('catalog/information', '&sort=id.title' . $url, 'SSL');
		$this->data['sort_sort_order'] = $this->url->link('catalog/information', '&sort=i.sort_order' . $url, 'SSL');
		$url                           = '';
		...
		$this->template           = 'template/catalog/information_list.tpl';
		$this->children           = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm()
	{
		...
		$this->data['entry_link_url']          = $this->language->get('entry_link_url');
		$this->data['entry_information_group'] = $this->language->get('entry_information_group');
		...
		{
			$this->data['action'] = $this->url->link('catalog/information/insert', $url, 'SSL');
		}
		else
		{
			$this->data['action'] = $this->url->link('catalog/information/update', '&information_id=' . $this->request->get['information_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('catalog/information', $url, 'SSL');
		...
		{
			$this->data['link_url'] = $this->request->post['link_url'];
		}
		elseif (!empty($information_info))
		{
			$this->data['link_url'] = $information_info['link_url'];
		}
		else
		{
			$this->data['link_url'] = '';
		...
		{
			$this->data['information_group_id'] = $this->request->post['information_group_id'];
		}
		elseif (!empty($information_info))
		{
			$this->data['information_group_id'] = $information_info['information_group_id'];
		}
		else
		{
			$this->data['information_group_id'] = 0;
		}

		$this->load->model('catalog/information_group');
		$this->data['information_groups'] = $this->model_catalog_information_group->getInformationGroups();
		...
		$this->template        = 'template/catalog/information_form.tpl';
		$this->children        = array(
			'common/header',
			'common/footer'
		);
