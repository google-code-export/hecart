...
class ControllerSaleVoucher extends Controller
{
	...
	public function insert()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm())
		{
			...
			$this->redirect($this->url->link('sale/voucher', $url, 'SSL'));
		}
	...
	private function getList()
	{
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('sale/voucher', $url, 'SSL'),
			'separator' => ' :: '
		);
		$this->data['insert']        = $this->url->link('sale/voucher/insert', $url, 'SSL');
		$this->data['delete']        = $this->url->link('sale/voucher/delete', $url, 'SSL');
		...
		foreach ($results as $result)
		{
			...
			$action[]                 = array(
				...
				'href' => $this->url->link('sale/voucher/update', '&voucher_id=' . $result['voucher_id'] . $url, 'SSL')
			);
			$this->data['vouchers'][] = array(
		...
		$this->data['sort_code']       = $this->url->link('sale/voucher', '&sort=v.code' . $url, 'SSL');
		$this->data['sort_from']       = $this->url->link('sale/voucher', '&sort=v.from_name' . $url, 'SSL');
		$this->data['sort_to']         = $this->url->link('sale/voucher', '&sort=v.to_name' . $url, 'SSL');
		$this->data['sort_theme']      = $this->url->link('sale/voucher', '&sort=theme' . $url, 'SSL');
		$this->data['sort_amount']     = $this->url->link('sale/voucher', '&sort=v.amount' . $url, 'SSL');
		$this->data['sort_status']     = $this->url->link('sale/voucher', '&sort=v.date_end' . $url, 'SSL');
		$this->data['sort_date_added'] = $this->url->link('sale/voucher', '&sort=v.date_added' . $url, 'SSL');
		$url                           = '';
		...
		$pagination->url          = $this->url->link('sale/voucher', $url . '&page={page}', 'SSL');
		$this->data['pagination'] = $pagination->render();
		$this->data['sort']       = $sort;
		$this->data['order']      = $order;
		$this->template           = 'template/sale/voucher_list.tpl';
		$this->children           = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm()
	{
		...
		if (!isset($this->request->get['voucher_id']))
		{
			$this->data['action'] = $this->url->link('sale/voucher/insert', $url, 'SSL');
		}
		else
		{
			$this->data['action'] = $this->url->link('sale/voucher/update', '&voucher_id=' . $this->request->get['voucher_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('sale/voucher', $url, 'SSL');
		...
		$this->template = 'template/sale/voucher_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validateForm()
	{
		...
		if ((mb_strlen($this->request->post['code']) < 3) || (mb_strlen($this->request->post['code']) > 10))
		{
		...
		if ((mb_strlen($this->request->post['to_name']) < 1) || (mb_strlen($this->request->post['to_name']) > 64))
		{
			$this->error['to_name'] = $this->language->get('error_to_name');
		}

		if ((mb_strlen($this->request->post['to_email']) > 96) || !preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['to_email']))
		{
			$this->error['to_email'] = $this->language->get('error_email');
		}

		if ((mb_strlen($this->request->post['from_name']) < 1) || (mb_strlen($this->request->post['from_name']) > 64))
		{
			$this->error['from_name'] = $this->language->get('error_from_name');
		}

		if ((mb_strlen($this->request->post['from_email']) > 96) || !preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['from_email']))
		{
	...
	private function validateDelete()
	{
		...
		foreach ($this->request->post['selected'] as $voucher_id)
		{
			...
			{
				$this->error['warning'] = sprintf($this->language->get('error_order'), $this->url->link('sale/order/info', '&osn=' . $order_voucher_info['order_id'], 'SSL'));
	...
	public function history()
	{
		...
		$pagination->page         = $page;
		$pagination->limit        = 10;
		$pagination->url          = $this->url->link('sale/voucher/history', '&voucher_id=' . $this->request->get['voucher_id'] . '&page={page}', 'SSL');
		$this->data['pagination'] = $pagination->render();
		$this->template           = 'template/sale/voucher_history.tpl';
		$this->response->setOutput($this->render());
	}
