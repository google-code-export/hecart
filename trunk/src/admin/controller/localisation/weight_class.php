...
class ControllerLocalisationWeightClass extends Controller
{
	...
	public function insert()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm())
		{
			...
			$this->redirect($this->url->link('localisation/weight_class', $url, 'SSL'));
		}
	...
	private function getList()
	{
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('localisation/weight_class', $url, 'SSL'),
			'separator' => ' :: '
		);
		$this->data['insert']        = $this->url->link('localisation/weight_class/insert', $url, 'SSL');
		$this->data['delete']        = $this->url->link('localisation/weight_class/delete', $url, 'SSL');
		...
		foreach ($results as $result)
		{
			...
			$action[]                       = array(
				...
				'href' => $this->url->link('localisation/weight_class/update', '&weight_class_id=' . $result['weight_class_id'] . $url, 'SSL')
			);
			$this->data['weight_classes'][] = array(
		...
		$this->data['sort_title'] = $this->url->link('localisation/weight_class', '&sort=title' . $url, 'SSL');
		$this->data['sort_unit']  = $this->url->link('localisation/weight_class', '&sort=unit' . $url, 'SSL');
		$this->data['sort_value'] = $this->url->link('localisation/weight_class', '&sort=value' . $url, 'SSL');
		$url                      = '';
		...
		$pagination->url          = $this->url->link('localisation/weight_class', $url . '&page={page}', 'SSL');
		$this->data['pagination'] = $pagination->render();
		$this->data['sort']       = $sort;
		$this->data['order']      = $order;
		$this->template           = 'template/localisation/weight_class_list.tpl';
		$this->children           = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm()
	{
		...
		{
			$this->data['action'] = $this->url->link('localisation/weight_class/insert', $url, 'SSL');
		}
		else
		{
			$this->data['action'] = $this->url->link('localisation/weight_class/update', '&weight_class_id=' . $this->request->get['weight_class_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('localisation/weight_class', $url, 'SSL');
		...
		$this->template = 'template/localisation/weight_class_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validateForm()
	{
		...
		{
			if ((mb_strlen($value['title']) < 3) || (mb_strlen($value['title']) > 32))
			{
