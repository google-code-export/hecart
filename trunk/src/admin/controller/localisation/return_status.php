...
class ControllerLocalisationReturnStatus extends Controller
{
	...
	public function insert()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm())
		{
			...
			$this->redirect($this->url->link('localisation/return_status', $url, 'SSL'));
		}
	...
	private function getList()
	{
		...
		$this->data['breadcrumbs'][]   = array(
			...
			'href'      => $this->url->link('localisation/return_status', $url, 'SSL'),
			'separator' => ' :: '
		);
		$this->data['insert']          = $this->url->link('localisation/return_status/insert', $url, 'SSL');
		$this->data['delete']          = $this->url->link('localisation/return_status/delete', $url, 'SSL');
		...
		foreach ($results as $result)
		{
			...
			$action[]                        = array(
				...
				'href' => $this->url->link('localisation/return_status/update', '&return_status_id=' . $result['return_status_id'] . $url, 'SSL')
			);
			$this->data['return_statuses'][] = array(
		...
		$this->data['sort_name'] = $this->url->link('localisation/return_status', '&sort=name' . $url, 'SSL');
		$url                     = '';
		...
		$pagination->url          = $this->url->link('localisation/return_status', $url . '&page={page}', 'SSL');
		$this->data['pagination'] = $pagination->render();
		$this->data['sort']       = $sort;
		$this->data['order']      = $order;
		$this->template           = 'template/localisation/return_status_list.tpl';
		$this->children           = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm()
	{
		...
		{
			$this->data['action'] = $this->url->link('localisation/return_status/insert', $url, 'SSL');
		}
		else
		{
			$this->data['action'] = $this->url->link('localisation/return_status/update', '&return_status_id=' . $this->request->get['return_status_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('localisation/return_status', $url, 'SSL');
		...
		$this->template = 'template/localisation/return_status_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validateForm()
	{
		...
		{
			if ((mb_strlen($value['name']) < 3) || (mb_strlen($value['name']) > 100))
			{
