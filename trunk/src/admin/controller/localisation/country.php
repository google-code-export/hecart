...
class ControllerLocalisationCountry extends Controller
{
	...
	public function insert()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm())
		{
			...
			$this->redirect($this->url->link('localisation/country', $url, 'SSL'));
		}
	...
	private function getList()
	{
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('localisation/country', $url, 'SSL'),
			'separator' => ' :: '
		);
		$this->data['insert']        = $this->url->link('localisation/country/insert', $url, 'SSL');
		$this->data['delete']        = $this->url->link('localisation/country/delete', $url, 'SSL');
		...
		foreach ($results as $result)
		{
			...
			$action[]                  = array(
				...
				'href' => $this->url->link('localisation/country/update', '&country_id=' . $result['country_id'] . $url, 'SSL')
			);
			$this->data['countries'][] = array(
		...
		$this->data['sort_name']       = $this->url->link('localisation/country', '&sort=name' . $url, 'SSL');
		$this->data['sort_iso_code_2'] = $this->url->link('localisation/country', '&sort=iso_code_2' . $url, 'SSL');
		$this->data['sort_iso_code_3'] = $this->url->link('localisation/country', '&sort=iso_code_3' . $url, 'SSL');
		$url                           = '';
		...
		$pagination->url          = $this->url->link('localisation/country', $url . '&page={page}', 'SSL');
		$this->data['pagination'] = $pagination->render();
		$this->data['sort']       = $sort;
		$this->data['order']      = $order;
		$this->template           = 'template/localisation/country_list.tpl';
		$this->children           = array(
			'common/header',
			'common/footer'
		);
	...
	private function getForm()
	{
		...
		{
			$this->data['action'] = $this->url->link('localisation/country/insert', $url, 'SSL');
		}
		else
		{
			$this->data['action'] = $this->url->link('localisation/country/update', '&country_id=' . $this->request->get['country_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('localisation/country', $url, 'SSL');
		...
		$this->template = 'template/localisation/country_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
