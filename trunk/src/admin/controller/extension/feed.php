...
class ControllerExtensionFeed extends Controller
{
	public function index()
	{
		...
		{
			if (!file_exists(DIR_SITE . '/controller/feed/' . $value . '.php'))
			{
		...
		$files                    = glob(DIR_SITE . '/controller/feed/*.php');
			...
			foreach ($files as $file)
			{
				...
				if (!in_array($extension, $extensions))
				{
					$action[] = array(
						...
						'href' => $this->url->link('extension/feed/install', '&extension=' . $extension, 'SSL')
					);
				}
				else
				{
					...
					$action[] = array(
						...
						'href' => $this->url->link('extension/feed/uninstall', '&extension=' . $extension, 'SSL')
					);
				}
		...
		$this->template = 'template/extension/feed.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function install()
	{
		...
		else
		{
			...
			require(DIR_SITE . '/controller/feed/' . $this->request->get['extension'] . '.php');
