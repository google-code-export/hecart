...
class ControllerSettingStore extends Controller
{
	...
	public function update()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm())
		{
			...
			$this->session->data['success'] = $this->language->get('text_success');
			$this->redirect($this->url->link('setting/store', '&store_id=' . $this->request->get['store_id'], 'SSL'));
		}
	...
	private function getList()
	{
		...
		foreach ($results as $result)
		{
			...
			$action[]               = array(
				'text' => $this->language->get('text_edit'),
				'href' => $this->url->link('setting/store/update', '&store_id=' . $result['store_id'], 'SSL')
			);
			$this->data['stores'][] = array(
		...
		$this->data['column_domain']   = $this->language->get('column_domain');
		...
		$this->template = 'template/setting/store_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function getForm()
	{
		...
		{
			$this->data['action'] = $this->url->link('setting/store/update', '&store_id=' . $this->request->get['store_id'], 'SSL');
		}
		...
		{
			$this->data['config_domain'] = $this->request->post['config_domain'];
		}
		elseif (isset($store_info['config_domain']))
		{
			$this->data['config_domain'] = $store_info['config_domain'];
		}
		else
		{
			$this->data['config_domain'] = '';
		}
		...
		$this->template = 'template/setting/store_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function template()
	{
		...
		{
			$image = $this->registry->cdn_img_url('themes/' . $template . '.png', IMAGES_PATH);
		}
		else
		{
			$image = $this->registry->cdn_img_url('no_image.jpg', IMAGES_PATH);
		}
