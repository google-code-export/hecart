...
class modules_mem extends wcore_object
{
	 ...
	 * @var string
	 */
	public $mem_type_asyn_sql = 'ASYN-SQL-DATA';
	...
	/**
	 * 异步增加SQL语句若增加不成功则执行SQL
	 *
	 * @param string $sql SQL语句
	 */
	public function asyn_add_sql($sql)
	{
		$num = $this->mem->increment($this->mem_type_asyn_sql, 'NUM');
		if (!$this->mem->set($this->mem_type_asyn_sql, "NUM{$num}", $sql))
		{
			$this->mdb()->query($sql);
		}

		/**
		 * 当异步数据大于等于100条时就马上执行缓冲中的所有SQL
		 */
		if ($num >= MEM_ASYN_SQL_NUM)
		{
			for ($i = 1; $i <= MEM_ASYN_SQL_NUM; $i++)
			{
				$sql = $this->mem->get($this->mem_type_asyn_sql, "NUM{$i}");
				$this->mdb()->query($sql);
				$this->mem->del($this->mem_type_asyn_sql, "NUM{$i}");
			}
			$val = 0;
			$this->mem->set($this->mem_type_asyn_sql, 'NUM', $val);
		}
	}
